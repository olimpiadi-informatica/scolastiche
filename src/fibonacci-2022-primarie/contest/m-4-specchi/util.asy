...
void griglia(int N, int M, pen border = defaultpen) {
	draw((0,0) -- (M*GRID_SIZE,0), border);
	draw((M*GRID_SIZE,0) -- (M*GRID_SIZE,N*GRID_SIZE), border);
	draw((M*GRID_SIZE,N*GRID_SIZE) -- (0,N*GRID_SIZE), border);
	draw((0,N*GRID_SIZE) -- (0,0), border);
	for (int i = 0; i < M-1; i+=1) {
		draw( ((i+1)*GRID_SIZE, 0) -- ((i+1)*GRID_SIZE, N*GRID_SIZE), linetype(new real[] {6,6})+border);
	}
	for (int i = 0; i < N-1; i+=1) {
		draw( (0,(i+1)*GRID_SIZE) -- (M*GRID_SIZE,(i+1)*GRID_SIZE), linetype(new real[] {6,6})+border);
	}
}
void numeri(int N, int M) {
	for (int i = 0; i < N; i+=1) {
		label(string(N-i-1), (-0.3,i*GRID_SIZE+GRID_SIZE/2));
		label(string(N-i-1), (M*GRID_SIZE+0.3,i*GRID_SIZE+GRID_SIZE/2));
	}
	for (int i = 0; i < M; i+=1) {
		label(string(i), (i*GRID_SIZE+GRID_SIZE/2,-0.3));
		label(string(i), (i*GRID_SIZE+GRID_SIZE/2,N*GRID_SIZE+0.3));
	}
}

    ...
    <>
      <Canvas scale={10}>
        ...
        {state.queue.flatMap((rabbit, i) => (
          <Sprite key={`rabbit${rabbit.height}`} src={teams.get(rabbit.farm)} alt={rabbit.farm} x={i*5 + 25} y={state.offset === i ? 0 : 10} scaleY={rabbit.height / 28} className="origin-bottom" />
        ))}
        {state.queue.flatMap((rabbit, i) => (
          <Sprite key={`carrot${rabbit.height}`} src={carrots.get(rabbit.farm)} alt="carrot" x={i*5 + 28.8} y={(state.offset === i ? 0 : 10) + 8.2 - rabbit.height*0.075} className="origin-bottom" />
        ))}
        {state.pairs.map((pair, y) => (
          pair.map((rabbit, x) => (
            <Sprite key={rabbit.height} src={teams.get(rabbit.farm)} alt={rabbit.farm} x={x*5} y={8*(y-state.pairs.length) + 18} scaleY={rabbit.height / 28} className="origin-bottom" />
          ))
        ))}
        {state.pairs.map((pair, y) => (
          pair.map((rabbit, x) => (
            <Sprite key={`carrot${rabbit.height}`} src={carrots.get(rabbit.farm)} alt="carrot" x={x*5 + 3.8} y={8*(y-state.pairs.length) + 18 + 8.2 - rabbit.height*0.075} className="origin-bottom" />
          ))
        ))}
      </Canvas>

        ...
        {range(state.N).map((i) =>
          range(state.N).map((j) => (
            <Rectangle
              ...
              x={5 + cellSide * i +1}
              y={-cellSide * j +1}>
              <Sprite key={`bush${i}-${j}`} src={content[`./asy/content${( ((i+state.N*j)*7+5)%33 + ((i+state.N*j)*11+8)%17 )%4}.asy`]} alt="cespuglio" x={cellSide * 0.2-1} y={cellSide * 0.17-1} />
            </Rectangle>
          )),
        )}
        <Sprite src={bunny} alt="Carol" x={3 + cellSide*state.x - 2.5*state.dy} y={-cellSide*state.y + 6 - 2.5*state.dx} rotation={state.a} />
        {range(state.eat ? 0 : 1).map((i) => (
          <Sprite src={carrot} alt="Carrot" x={cellSide*state.N + 3.5} y={-cellSide*state.N + 8.0} />
        ))}
      </Canvas>

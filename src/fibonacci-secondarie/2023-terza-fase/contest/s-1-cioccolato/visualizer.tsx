...
const bunnies: Record<string, ImageSrc> = import.meta.glob("./asy/bunnies*.asy", {
  eager: true,
  import: "default",
  query: { w: 50 },
});
...
export default function Visualizer({ variables, state }: VisualizerProps<State>) {
  ...
  const xChoco = range(N).map((): number[] => []);
  const yChoco = range(M).map((): number[] => []);

  let xoffs = 1.5;
  let yoffs = 2;

  const chocblock = (N0: number, N: number, M0: number, M: number) => {
    for (let i = N0; i < N; ++i) {
      xChoco[i] ??= [];
      yChoco[i] ??= [];
      for (let j = M0; j < M; ++j) {

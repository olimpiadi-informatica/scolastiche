...
export default [
  ...
  },
  {
    ...
    message0: "N",
    colour: 20,
    tooltip: "numero di stanze nella fattoria",
  ...
  },
  {
    ...
    message0: "M",
    colour: 20,
    tooltip: "numero di coniglietti da recuperare",
    fn: (_ctx: Context, state: State): number => {
      return state.M;
    },
  },
  ...
  },
  {
    ...
    fn: (ctx: Context, state: State, v1$: "destra" | "sinistra"): boolean => {
      ...
      state.dir = v1;
      if (state.pos + v1 > state.N) ctx.exit(false, "non c'è una stanza a destra");
      if (state.pos + v1 < 1) ctx.exit(false, "non c'è una stanza a sinistra");
      return state.light[state.pos + v1 - 1] === 1;
    },
  },
  {
    type: "interruttore",
    ...
    fn: (ctx: Context, state: State, v1$: "destra" | "sinistra") => {
      ...
      if (state.pos + v1 > state.N) ctx.exit(false, "non c'è una stanza a destra");
      if (state.pos + v1 < 1) ctx.exit(false, "non c'è una stanza a sinistra");
      state.switch[2 * state.pos - 1 + Math.max(v1, 0)] ^= 1;
      state.light[state.pos + v1 - 1] ^= 1;
    },
  },
  {
    type: "termina",

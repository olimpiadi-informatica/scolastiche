...
export default [
  ...
  {
    type: "costo_delle_box_positive",
    message0: "costo delle box positive",
    colour: 20,
    tooltip: "costo in carote delle box per alzare i punteggi dei giocatori",
    fn: (_ctx: Context, state: State): number => {
      return state.K;
    },
  },
  ...
  {
    type: "compra_box_positiva",
    message0: "compra box positiva %1",
    previousStatement: null,
    nextStatement: null,
    colour: 20,
    tooltip:
      "paga la box positiva i-esima e alza il punteggio del giocatore che al momento è i-esimo",
    fn: async (ctx: Context, state: State, v1: number) => {
      ...
      state.pos = v1;
      if (state.allie === 0)
        ctx.exit(false, "Allie è già prima in classifica, non serve alzarle il punteggio");
      if (v1 !== state.allie + 1)
        ctx.exit(false, "hai comprato la box per alzare il punteggio di un avversario di Allie");
      state.P[v1 - 1][0]++;
      state.count += state.K;
      state.highlight = 1;
      await ctx.pause();
      if (state.P[v1 - 1][0] > state.P[v1 - 2][0]) {
        state.stress[v1 - 1] = -1;
        let j = v1 - 2;
        state.stress[j] = +1;
        for (; j >= 1 && state.P[j][0] === state.P[j - 1][0]; j--) state.stress[j - 1] = +1;
        await ctx.pause();
        const tmp = state.P[v1 - 1];
        for (let k = v1 - 2; k >= j; k--) {
          state.P[k + 1] = state.P[k];
          state.stress[k + 1] = 0;
        }
        state.P[j] = tmp;
        state.stress[j] = 3 + 4 * (j - v1);
        state.allie = j;
        await ctx.pause();
        state.stress[j] = 0;
      }

...
export default [
  ...
  },
  {
    ...
    fn: (ctx: Context, state: State, v1: string) => {
      ...
      state.trash = false;
      if (
        state.food_types[food[0]] === "verdura" &&
        state.food_colors[food[0]] !== "giallo" &&
        food.length === 1
      )
        ctx.exit(false, "a Carol non piace la verdura cruda se non è gialla");
      if (
        state.food_types[food[0]] === "verdura" &&
        state.food_colors[food[0]] === "giallo" &&
        food.length > 1
      )
        ctx.exit(false, "a Carol non piace la verdura gialla cotta");
      if (state.food_types[food[0]] === "frutta" && food.length > 1)
        ctx.exit(false, "a Carol non piace la frutta cotta");
    },
  },
  {
    type: "butta",
    ...
    fn: (ctx: Context, state: State, v1: string) => {
      ...
      state.pos += 1;
      state.trash = true;
      ctx.exit(false, "a Carol piace tutto, non dovevi buttare via questo");
    },
  },
  {
    type: "termina",

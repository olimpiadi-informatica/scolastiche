...
export default [
  ...
  {
    type: "mele_prossimo_albero",
    message0: "mele prossimo albero",
    colour: 20,
    tooltip: "numero di mele nell'albero di fronte a Tip-Tap",
    fn: (ctx: Context, state: State): number => {
      if (state.pos === state.N) ctx.exit(false, "non ci sono alberi di fronte a Tip-Tap");
      return state.M[state.pos];
    },
  },
  {
    type: "avanza",
    ...
    fn: (ctx: Context, state: State) => {
      if (state.pos === state.N) ctx.exit(false, "sei già alla fine degli alberi");
      state.pos += 1;
    },
  },
  {

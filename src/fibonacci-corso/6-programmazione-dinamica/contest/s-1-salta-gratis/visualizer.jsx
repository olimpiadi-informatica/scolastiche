...
export default function Visualizer({ variables, state }) {
  ...
  return (
    <>
      <Canvas scale={130}>
        {state.start == 1 ? null : (
          <Sprite src={bunny} alt="Tip-Tap" x={state.pos + 0.2} y={0.2-0.6*state.start} follow />
        )}
        {range(state.N).map((i) => {
          return (
            <Sprite
        ...
        {range(state.N).map((i) => state.cost[i] == 0 ? null : (
          <Rectangle
            color="transparent"
            width={0.5}
            height={0.3}
      ...
      <Variables variables={{ ...variables, "N (num. rocce)" : state.N, "carote spese": state.count }} />
    </>
  );
}

...
- `costo roccia` $i$: il costo $P_i$ in carote per atterrare o avanzare sulla $i$-esima roccia del percorso (zero per le rocce gratis).

In aggiunta, hai due blocchi nuovi per leggere e scrivere sulle lavagne:
...
Hai anche gli stessi blocchi di prima per fare la corsa:
...
> Se siamo abituati a pensare in modo greedy, una cosa che ci può venire in mente di fare è di concludere
> la corsa facendo solo salti, per toccare meno pietre a pagamento possibile.
>
> ![sbagliata](wrong.png)
>
> Questo procedimento, però, non sempre ci porta al risultato migliore: a volte conviene avanzare su
> una roccia davanti a sé, per poter saltare quella dopo molto più cara. Non possiamo scegliere sul
> momento senza preoccuparci delle conseguenze: dobbiamo sfruttare i nuovi blocchi!
>
> L'idea principale che serve a risolvere il problema è quindi capire cosa annotare sulle lavagne. Supponiamo
> di scrivere su ogni lavagna un numero che rappresenta **quanto ci costa finire il percorso da quel punto in poi**.
> Con questa idea, un possibile programma corretto è il seguente:
>
> ![soluzione](sol.png)
>
> Secondo questo programma, per scrivere sulle lavagne, le scandiamo dall'ultima tornando indietro verso la prima.
> Per ciascuna di esse dobbiamo scrivere quanto ci costa finire il percorso da lì. Ci viene anche in aiuto che

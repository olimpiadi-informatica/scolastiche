...
export default [
  ...
  },
  {
    ...
    fn: (ctx: Context, state: State, v1: number, v2: number) => {
      ...
      if (v2 < 1 || v2 > state.N) ctx.exit(false, `la lavagna ${v2} non esiste`);
  ...
  },
  {
    ...
    fn: (ctx: Context, state: State, v1: number): number => {
      if (v1 < 1) ctx.exit(false, "il parametro deve essere maggiore o uguale di 1");
      if (!Number.isInteger(v1)) ctx.exit(false, "il parametro deve essere un intero");
      if (v1 < 1 || v1 > state.N) return 0;
      return state.value[v1 - 1];
    },
  },
  {
    type: "comincia",
  ...
  },
  {
    ...
    fn: (ctx: Context, state: State) => {
      ...
      if (state.count > state.sol) ctx.exit(false, "potevi spendere meno carote");
      ctx.exit(true, "hai finito la corsa spendendo poco, complimenti!");
    },
  },

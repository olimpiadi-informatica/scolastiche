...
export default [
  ...
  },
  {
    ...
    fn: (ctx: Context, state: State, v1: number, v2: number): number => {
      ...
      if (v2 > state.N) ctx.exit(false, `il giorno ${v2} non esiste`);
      if (v2 <= v1) ctx.exit(false, `il giorno ${v2} è prima della validità del titolo ${v1}`);
      return state.V[v1] + (v2 - v1) * state.G[v1];
    },
  },
  {

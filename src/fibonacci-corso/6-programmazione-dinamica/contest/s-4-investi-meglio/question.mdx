...
Inoltre, hai a disposizione questi nuovi blocchi per annotare informazioni di supporto:

- `scrivi` $x$ `sul titolo` $i$: scrivi un numero $x$ a tua scelta sul titolo $i$-esimo **(nuovo!)**.
- `preferenza per il titolo` $i$: leggi il numero che hai scritto sul titolo $i$-esimo **(nuovo!)**.
...
<Hint label="suggerimento 3">
  In un solo ciclo contatore, puoi scrivere il valore di preferenza del titolo $i$-esimo
...
> ogni giorno, se possiamo scambiare il titolo che abbiamo con quello del giorno, e quello del giorno
> ci può dare una rendita migliore alla fine, facciamo lo scambio.
>
> ![sbagliata](wrong.png)
>
> Questo procedimento, però, non sempre ci porta al risultato migliore: a volte conviene evitare uno
> scambio potenzialmente conveniente, che però ci fa perdere valore subito, per poterne fare uno ancora
> più conveniente poco dopo. Non possiamo scegliere sul momento senza preoccuparci delle conseguenze:
> dobbiamo sfruttare i nuovi blocchi!
>
> L'idea principale che serve a risolvere il problema è quindi capire cosa scrivere sui titoli. Supponiamo
> di scrivere su ogni titolo un numero che rappresenta **quanto al massimo potremmo avere all'ultimo giorno**,
...
> cosa possiamo scriverci sopra (e quale "freccia" mettere). Per l'ultimo ci sono poche scelte: dovremo per
> forza tenerlo, quindi non mettiamo frecce e scriviamo il valore che acquisisce all'ultimo giorno.
>
> Per gli altri titoli $i$, iniziamo comunque allo stesso modo, considerando cosa possiamo ottenere se li tenessimo
> fino alla fine, quindi scrivendo il valore che acquisirebbero all'ultimo giorno. Poi, però, dobbiamo consideriare
